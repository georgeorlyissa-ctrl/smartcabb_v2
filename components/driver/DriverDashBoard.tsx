...
export function DriverDashboard() {
  ...
  const handleCompleteRide = async () => {
    if (state.currentRide && rideStartTime) {
      ...
      try {
        // ✅ FIX : utiliser l'ID réel de la course (UUID du backend) au lieu d'en générer un nouveau
        const realRideId = state.currentRide?.id || rideRequest?.id || `ride_${driver.id}_${Date.now()}`;
        console.log('💾 Sauvegarde course dans le backend avec ID:', realRideId);
