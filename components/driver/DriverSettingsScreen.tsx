...
const CarIc       = ({ className = 'w-5 h-5' }: { className?: string }) => (<svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1M5 17a2 2 0 104 0m-4 0a2 2 0 114 0m6 0a2 2 0 104 0m-4 0a2 2 0 114 0"/></svg>);
...
const UserIc      = ({ className = 'w-5 h-5' }: { className?: string }) => (<svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>);
...
/* ─────────────────────────────────────────────────────────────────────
   AccordionSection — composant isolé avec son propre state d'ouverture
   Utilise useRef pour manipuler le DOM directement → zéro dépendance
   au state du parent, zéro re-render parasite
───────────────────────────────────────────────────────────────────── */
...
interface AccordionSectionProps {
  ...
  iconColor: string;
  items: AccordionItem[];
  getVal: (cat: string, key: string) => boolean;
  setVal: (cat: string, key: string, v: boolean) => void;
}

function AccordionSection({ title, icon: Icon, color, iconColor, items, getVal, setVal }: AccordionSectionProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      >
        ...
        <span
          ref={chevronRef}
          style={{ transition: 'transform 0.25s ease', display: 'inline-flex', color: '#9ca3af' }}
        >
          <ChevronDown className="w-5 h-5" />
        </span>
      </button>

      {/* Contenu — height animée via ref, pas via state */}
      <div
        ref={contentRef}
        style={{ maxHeight: '0px', overflow: 'hidden', visibility: 'hidden', transition: 'max-height 0.25s ease' }}
      >
        <div className="border-t border-gray-100">
          {items.map(({ key, label, description, category }, idx) => (
            <div
              key={key}
            ...
            >
              ...
              <Switch
                checked={getVal(category, key)}
                onCheckedChange={(v) => setVal(category, key, v)}
              />
            </div>
          ))}
        </div>
...
export function DriverSettingsScreen() {
  ...
  const getVal = (cat: string, key: string): boolean =>
    !!((settings as any)[cat]?.[key]);

  const sections = [
    {
      title: 'Notifications', icon: Bell, color: 'bg-orange-100', iconColor: 'text-orange-500',
      items: [
        { key: 'newRides',    label: 'Nouvelles demandes',   description: 'Notifications pour les nouvelles courses',       category: 'notifications' },
        { key: 'payments',   label: 'Paiements reçus',      description: 'Notifications de confirmation de paiement',      category: 'notifications' },
        { key: 'promotions', label: 'Promotions et offres', description: 'Informations sur les promotions',                 category: 'notifications' },
    ...
    },
    {
      ...
      items: [
        ...
        { key: 'allowDirectContact', label: 'Contact direct',       description: 'Permettre aux clients de vous contacter',             category: 'privacy' },
      ],
    },
    {
      title: 'Préférences', icon: SettingsIc, color: 'bg-purple-100', iconColor: 'text-purple-600',
      items: [
        { key: 'darkMode',     label: 'Mode sombre',   description: "Thème sombre de l'application", category: 'preferences' },
        { key: 'soundEffects', label: 'Effets sonores', description: 'Sons de notification',           category: 'preferences' },
      ],
    },
  ];
  ...
  const quickActions = [
    ...
    { title: 'Informations véhicule', description: 'Modifier les détails du véhicule',       icon: CarIc  },
    { title: 'Paramètres de gain',    description: 'Configuration des revenus',              icon: DollarSign },
    ...
    <div className="min-h-screen bg-gray-50 pb-10">
      ...
      <div className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-10">
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            <button onClick={() => setCurrentScreen('driver-dashboard')} className="p-2 rounded-full hover:bg-gray-100 transition-colors">
          ...
          >
            ...
            <span className="text-xs font-medium text-gray-600">{settings.preferences.darkMode ? 'Clair' : 'Sombre'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            {quickActions.map(({ title, description, icon: Icon }) => (
              <button key={title} type="button" className="flex flex-col items-start gap-1 p-3 rounded-xl border border-gray-100 bg-gray-50 hover:bg-blue-50 hover:border-blue-200 transition-colors text-left">
        ...
        {/* Accordéons — chacun est un composant isolé */}
        {sections.map((s) => (
          <AccordionSection
            key={s.title}
            title={s.title}
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 space-y-4">
          ...
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
            ...
            <div className="flex items-center gap-2">
              <input type="range" min="1" max="20" value={settings.preferences.autoAcceptRadius}
